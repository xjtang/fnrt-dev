// Snapshot of users/andreasvollrath/radar:slope_correction_lib.js on 11/4/2020 (reformated)
...
var slope_correction = function (collection, options) {
  ...
  function _surface_model(theta_iRad, alpha_rRad, alpha_azRad) {
    ...
    var denominator = alpha_azRad.cos().multiply((ninetyRad.subtract(theta_iRad)
                                  .add(alpha_rRad)).cos());
    return nominator.divide(denominator);
  }
  ...
  function _erode(img, distance) {
    var d = (img.not().unmask(1).fastDistanceTransform(30).sqrt()
  ...
  function _correct(image) {
    ...
    var heading = ee.Terrain.aspect(image.select('angle'))
                                      .reduceRegion(ee.Reducer.mean(), geom, 1000).get('aspect');
    ...
    // models
    if (model == 'volume') {var corrModel = _volume_model(theta_iRad, alpha_rRad)}
    if (model == 'surface') {var corrModel = _surface_model(theta_iRad, alpha_rRad, alpha_azRad)}
    if (model == 'direct') {var corrModel = _direct_model(theta_iRad, alpha_rRad, alpha_azRad)}

    // apply model for Gamm0_f
    var gamma0_flat = gamma0.divide(corrModel);

    // transform to dB-scale
    var gamma0_flatDB = (ee.Image.constant(10).multiply(gamma0_flat.log10()).select(['VV', 'VH']) );
...
var slope_correction_image = function (image, options) {
  ...
  function _correct(image){
    ...
    // return gamma_flat plus mask
    return gamma0_flat.addBands(image.select('angle')).addBands(mask).copyProperties(image)
